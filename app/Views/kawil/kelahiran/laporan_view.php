...
<script>
    function ViewTabelLaporan() {
        ...
        if (bulan == "") {
            ...
            $.ajax({
                type: "POST",
                url: "<?= base_url('Kelahiran/ViewLaporanBulanan') ?>",
                data: {
                    bulan: bulan,
                    tahun: tahun,
                },
    ...
    function PrintLaporan() {
        ...
        if (bulan == "") {
            ...
            NewWin = window.open('<?= base_url('Kelahiran/Print/') ?>/' + bulan + '/' + tahun, 'NewWin', 'toolbar=no, width=1500,height=800,scrollbars=yes');
